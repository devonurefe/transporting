    ...
    <div className="relative min-h-[calc(100vh-3.5rem)] py-6 sm:py-10 px-5 sm:px-6 lg:px-8">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="text-right shrink-0">
                            {machine.oneDayPrice && machine.oneDayPrice < machine.pricePerDay ? (
                              <>
                                <span className="inline-flex items-center rounded-full bg-gradient-to-r from-red-600 to-orange-600 text-white text-[10px] font-black uppercase tracking-wide px-1.5 py-0.5">Dagactie</span>
      ...
      <AnimatePresence>
        {pricingPreviewMachine && (() => {
          ...
          const rows = buildPricingTierRows(m).map(row => ({
            ...row,
            priceText: row.pricePrefix ? `${row.pricePrefix}${formatPrice(vp(row.price))}` : undefined,
          }));
