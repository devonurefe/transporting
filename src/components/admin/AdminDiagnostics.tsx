...
export default function AdminDiagnostics({ systemLogs, userProfiles, onAddSystemLog, adminLanguage }: AdminDiagnosticsProps) {
  ...
  const triggerSelfAudit = () => {
    ...
    const steps = [
      ...
      { text: t("[DB]    SQLite database indexering valideren op 'Machine' en 'Order'... [OK - 7 tabellen verified]", "[DB]    Validating SQLite database indexing on 'Machine' and 'Order'... [OK - 7 tables verified]", "[DB]    'Machine' ve 'Order' üzerinde SQLite veri tabanı indekslemesi doğrulanıyor... [OK - 7 tablo doğrulandı]"), delay: 1200 },
      ...
      { text: t("[API]   Verbinding testen met Google Gemini-1.5-Flash Pro model... [OK - Latency 142ms]", "[API]   Testing connection to Google Gemini-1.5-Flash Pro model... [OK - Latency 142ms]", "[API]   Google Gemini-1.5-Flash Pro modeli ile bağlantı test ediliyor... [OK - Gecikme 142ms]"), delay: 1900 },
